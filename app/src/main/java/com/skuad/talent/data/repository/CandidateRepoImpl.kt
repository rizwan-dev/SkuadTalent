...
import com.skuad.talent.data.mapper.changestate.ChangeStateMapper
...
class CandidateRepoImpl(private val apolloClient: ApolloClient) : CandidateRepo {
    ...
    override suspend fun changeState(changeStateRequest: ChangeStateRequest): ResourceState<ChangeStateResponse> {
        return apolloClient.mutate(ChangeStateCandidateMutation.builder().candidate_uid(changeStateRequest.candidateUid).stage(changeStateRequest.stage).build()).mapToEntity {
            ChangeStateMapper().map(it)
        }
    }
